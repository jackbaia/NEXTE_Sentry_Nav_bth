...
void PX4CtrlFSM::process()
{
	...
	// if(param.use_attitude_ctrl)
	// {
	// 	publish_attitude_ctrl(u, now_time);
	// }
	
	// if(param.use_pos_ctrl)
	// {
		
	// 	publish_pos_ctrl(des);
...
void PX4CtrlFSM::publish_pos_ctrl(const Desired_State_t &des, const ros::Time &stamp)
{

	geometry_msgs::PoseStamped pose;
	pose.header.stamp = stamp;
	pose.header.frame_id = std::string("FCU");
    pose.pose.position.x = des.p(0);
    pose.pose.position.y = des.p(1);
    pose.pose.position.z = des.p(2);

	tf::Quaternion quaternion = tf::createQuaternionFromYaw(des.yaw);
	pose.pose.orientation.x = quaternion.x();
	pose.pose.orientation.y = quaternion.y();
	pose.pose.orientation.z = quaternion.z();
	pose.pose.orientation.w = quaternion.w();

	sp_pos_pub.publish(pose);
